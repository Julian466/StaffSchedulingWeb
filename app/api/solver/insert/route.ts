...
import { executeInsert } from '@/lib/services/python-cli-service';
import { jobRepository } from '@/features/solver/api/job-repository';
import { InsertParams, SolverJob } from '@/types/solver';
import { randomUUID } from 'crypto';

const apiLogger = createApiLogger('/api/solver/insert');

/**
 * POST /api/solver/insert
 * Inserts data from JSON solution files to DB.
 * 
 * @body { unit: number, start: string (ISO 8601), end: string (ISO 8601) }
...
export async function POST(request: NextRequest) {
  try {
    ...
    apiLogger.info('Executing insert command', { caseId, params });
    ...
    return NextResponse.json(
      {
        error: 'Failed to execute insert command',
