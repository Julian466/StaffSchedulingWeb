...
| `solverMode` | Solver communication mode: `"cli"` for direct CLI execution, or `"api"` for REST API (with automatic server startup). Default: `"cli"`. See [Solver Integration](./solver-integration#configuring-the-solver-mode) for details. |

> **Important:** If you use `solverMode: "api"`, `npm run dev` / `npm run start` will automatically start the solver API using `staffSchedulingProject.path` and `staffSchedulingProject.pythonExecutable`.
    Make sure these fields are configured correctly, and point `casesDirectory` to the shared case folder between web app and solver.
